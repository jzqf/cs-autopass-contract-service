...
//        Database db = new Database();
        ...
        try {
            ...
            data.registerDriver();
            dbConnection = data.getConnection(conStr);
            logger.info("Setting catalog to ServerCommon");
            dbConnection.setCatalog("ServerCommon");
         
            Map MethodResult;
            MethodResult = data.paymentMethodGet(dbConnection, ClientNumber, AccountNumber, InvoiceNumber, SystemActorID, Username, Password);
                          
            if(MethodResult.get("ErrorCode").toString().equals("0")) {
                response.paymentMethodID =  Integer.parseInt(MethodResult.get("PaymentMethodID").toString());
                response.paymentMethodName = MethodResult.get("PaymentMethod").toString();
                response.errorCode = 0;
            }            
            else {
                response.errorCode = Integer.parseInt(MethodResult.get("ErrorCode").toString());
                response.errorMessage = MethodResult.get("ErrorMessage").toString();
            }
    ...
    public PaymentMethodUpdate paymentMethodUpdate(@XmlElement(required=true) @WebParam(name = "Username") String Username,
...
//     Database db = new Database();
        ...
        try {
            ...
            Map MethodResult;
            MethodResult = data.paymentMethodUpdate(dbConnection, ClientNumber, AccountNumber, InvoiceNumber, PaymentMethodID, SystemActorID, Username, Password);
...
//        try {
//        	
//             ResourceBundle rb = ResourceBundle.getBundle("PaymentMethod");
//        
...
//        }
//        catch(Exception e) {
//			logger.error("An exception was thrown:", e);	// EXCEPTION THROWN HERE. FIX THIS!!!!!!!!!!!!!!
//        }
